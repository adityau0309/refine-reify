      ...
      <section className="section-padding pt-28 md:pt-36">
        <SectionContainer>
          <SectionHeader
            ...
            highlight="paid."
            subtitle="Writing on accounts receivable operations, payment blockers and cash flow."
            align="center"
          />
          <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {blogPosts.map((post) => (
              <Link key={post.slug} to={`/blog/${post.slug}`} className="group rounded-3xl border border-border bg-card p-6 md:p-8 transition-all hover:border-primary hover:shadow-lg">
                <span className="kicker text-primary text-xs font-semibold">{post.category}</span>
                <h3 className="font-display text-xl font-bold mt-4 group-hover:text-primary transition-colors">{post.title}</h3>
          ...
          <div className="mt-12 flex flex-col items-center gap-4">
            ...
            <Link to="/" className="kicker hover:text-primary">Back to home</Link>
          </div>
        </SectionContainer>
      </section>
      <WhatsAppButton />
